...
function extractManualsAndGuides($) {
  // 1) Find the Manuals & Guides header
  const h2 = $("h2")
  ...
  // 2) Anchor to the actual Manuals & Guides BOX (not some random parent div)
  // The block you provided lives inside: <div class="bg-gray-10 ..."> ... <h2>Manuals & Guides</h2> ...
  const box = h2.closest("div.bg-gray-10");
  const scope = box.length ? box : h2.parent(); // fallback if class ever changes

  // 3) Iterate PDFs in the exact visual order (block-by-block)
  const manuals = [];
  let primary = null;

  // We use .block wrappers first because that matches the real list order
  const blocks = scope.find("div.block.align-top");
  const linkEls = blocks.length ? blocks.find("a[href$='.pdf']") : scope.find("a[href$='.pdf']");

  linkEls.each((_, a) => {
    const $a = $(a);

    const url = $a.attr("href");
    const title = normText($a.text()); // includes inner div text
    if (!url || !title) return;

    // HARD EXCLUSIONS (NO "instruction" HERE — that was killing "Installation Instructions")
    if (
      title.includes("quick") ||
      title.includes("owner") ||
      title.includes("owners") ||
    ...
    // Classify only what we care about
    let type = null;

    // IMPORTANT: installation-first classification
    if (
    ...
    const entry = { type, title, url };
    manuals.push(entry);

    // FIRST VALID DOC WINS (true order)
  ...
  return {
    primary, // ← whichever came first: install/planning OR spec/specification
    installation: manuals.find(m => m.type === "installation") || null,
    specification: manuals.find(m => m.type === "specification") || null
  };
}
