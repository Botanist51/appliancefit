...
function extractDimsFromMainPage($, sectionTitles, labelMatchers) {
  ...
  // We now search BOTH main page and specs tab,
  // but we will IGNORE cutout / opening sections
  const root = $("body");

  let header = null;
  root.find("h1,h2,h3,h4,div.bold,div.font-bold,strong").each((_, el) => {
    const t = normKey($(el).text());
    if (sectionTitleKeys.includes(t)) {
      header = $(el);
      return false;
  ...
  while (node.length) {
    const headerText = normKey(node.text());
    ...
    if (
      ...
      if (!sectionTitleKeys.includes(headerText)) break;
    }

    // If we wandered into a cutout section, skip
    if (isCutoutSection(headerText)) break;
    ...
    const labelKey = normKey(labelRaw);
...
function extractOverallDimensionsFromMainPage($) {
  const sectionTitles = [
    "Dimensions",
    "Product Dimensions",
    "Overall Dimensions",
  ...
  return extractDimsFromMainPage($, sectionTitles, labelMatchers);
}
...
// 1) MAIN PRODUCT PAGE FIRST (sidebar/front-row “Dimensions”)
const overallMain = extractOverallDimensionsFromMainPage($);
overallW = overallMain.width || null;
overallH = overallMain.height || null;
overallD = overallMain.depth || null;

// 2) FALLBACK: Specs tab “Dimensions” section (same engine cutout uses)
if (!overallW || !overallH || !overallD) {
  const overallFromSpecsTab = extractOverallDimensions($);
  if (!overallW && overallFromSpecsTab.width) overallW = overallFromSpecsTab.width;
  if (!overallH && overallFromSpecsTab.height) overallH = overallFromSpecsTab.height;
  if (!overallD && overallFromSpecsTab.depth) overallD = overallFromSpecsTab.depth;
}

// 3) LAST RESORT: pageText labels
if (!overallW) {
  overallW =
