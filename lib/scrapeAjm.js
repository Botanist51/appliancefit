...
function extractCutoutDimensions($) {
  ...
  // Find the exact "Cut Out Dimensions" header row
  const headerRow = $("tr").filter((_, el) => {
    return $(el).text().trim().toLowerCase() === "cut out dimensions";
  }).first();
  ...
  while (row.length) {
    ...
    else break; // stop once section ends
