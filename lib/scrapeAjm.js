...
function extractOverallFromSpecsTab($) {
  ...
  let minHeightRaw = null;
  let maxHeightRaw = null;
...
if (parsed === null) continue;
    ...
    // Capture min/max height rows (we'll build a range after the loop)
if (labelKey === "minimum height") {
  minHeightRaw = valueRaw.trim();
  continue;
}

if (labelKey === "maximum height") {
  maxHeightRaw = valueRaw.trim();
  continue;
}
...
if (
  ...
  // If we found min/max height, prefer a real range
if (minHeight && maxHeight) {
  const min = parseInches(minHeight);
  const max = parseInches(maxHeight);

  if (min !== null && max !== null && max > min) {
    out.height = `${min} to ${max}`;
  } else {
    out.height = minHeight;
  }
}

// If we DIDN'T already find a normal Height row, but we have BOTH min+max,
// then we can safely build a ranged height.
if (!out.height && minHeightRaw && maxHeightRaw) {
  const r = parseInchesRange(`${minHeightRaw} to ${maxHeightRaw}`);
  if (r.min !== null && r.max !== null && r.max > r.min) {
    out.height = `${r.min} to ${r.max}`;
  } else if (r.min !== null) {
    out.height = minHeightRaw; // fallback, but only if both existed originally
  }
}
