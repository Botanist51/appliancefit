...
function extractDimsFromQuickSpecs($) {
  const result = { width: null, height: null, depth: null };

  const root = $("#quick_specs");
  if (!root.length) return result;

  // Quick specs rows are typically a div containing a bold label span and then
  // either a text node or nested spans containing the value.
  root.find("span.bold").each((_, el) => {
    const $label = $(el);
    const labelKey = normKey($label.text().replace(":", ""));
    if (!labelKey) return;

    // Grab the text of the parent row and remove the label portion.
    const $row = $label.closest("div");
    if (!$row.length) return;

    const rowText = normText($row.text());
    const labelText = normText($label.text().replace(":", ""));

    // value = rowText - labelText (first occurrence)
    let value = rowText;
    if (labelText && value.startsWith(labelText)) {
      value = value.slice(labelText.length).trim();
    } else {
      // fallback: remove anywhere in the string (in case layout changes)
      value = value.replace(labelText, "").trim();
    }

    if (!value) return;

    // Accept common synonyms too (some pages use "overall width" etc. here)
    const isWidth =
  labelKey === "width" ||
  labelKey === "overall width" ||
  labelKey === "product width" ||
  labelKey === "actual width";

const isHeight =
  labelKey === "height" ||
  labelKey === "overall height" ||
  labelKey === "product height" ||
  labelKey === "actual height";

const isDepth =
  labelKey === "depth" ||
  labelKey === "overall depth" ||
  labelKey === "product depth" ||
  labelKey === "actual depth";

    if (!result.width && isWidth) result.width = value;
    if (!result.height && isHeight) result.height = value;
    if (!result.depth && isDepth) result.depth = value;
  });
...
function parseInchesRange(value) {
  ...
  // FIX: normalize hyphenated fractions FIRST
  raw = raw.replace(/(\d+)-(\d+\/\d+)/g, "$1 $2");

  // NOW normalize real ranges
...
export async function scrapeAjmModel(rawModel) {
  ...
  if (!overallW || !overallH || !overallD) {
    const overall = extractOverallDimensions($);
  ...
  // FORCE Quick Specs to fill any missing overall dimensions
const quick = extractDimsFromQuickSpecs($);
if (!overallW && quick.width) overallW = quick.width;
if (!overallH && quick.height) overallH = quick.height;
if (!overallD && quick.depth) overallD = quick.depth;

  // NEW: Fallback to main page if any overall dim is still missing
  if (!overallW || !overallH || !overallD) {
    ...
    const mainLabelMatchers = {
      width: ["width", "overall width", "product width", "unit width", "actual width"],
      height: ["height", "heigh", "overall height", "product height", "unit height", "actual height"],
      depth: ["depth", "overall depth", "product depth", "unit depth", "actual depth"]
    };
  ...
  // Last-resort pageText aliases (covers “Product Width:” type strings in body text)
