...
// Handle combined dimension formats like:
// "Product Dimensions (W x H x D): 36\" x 2 3/8\" x 21\""
if (!(result.width && result.height && result.depth)) {
  const looksLikeCombined =
    labelKey.includes("dimensions") &&
    (
      labelKey.includes("w x h x d") ||
      labelKey.includes("h x w x d") ||
      labelKey.includes("w x d x h") ||
      labelKey.includes("dimensions w")
    );
...
function extractDimsFromQuickSpecs($) {
  ...
  root.find("span.bold").each((_, el) => {
    const label = normKey($(el).text().replace(":", ""));
    const value = normText($(el)[0].nextSibling?.nodeValue || "");

    if (!label || !value) return;

    if (!result.width && label === "width") result.width = value;
    if (!result.height && label === "height") result.height = value;
    if (!result.depth && label === "depth") result.depth = value;
  });
...
if (!dimsComplete(overall)) {
  const quick = extractDimsFromQuickSpecs($);
  if (!overall.width && quick.width) overall.width = quick.width;
  if (!overall.height && quick.height) overall.height = quick.height;
  if (!overall.depth && quick.depth) overall.depth = quick.depth;
}
