...
if (!overallD) overallD = pick(pageText, "Product Depth") || pick(pageText, "Depth");
  ...
  const out = {
    ...
    "Overall Width (in)":
  overallWidthDec !== null ? fmtNum(overallWidthDec) : "N/A",

"Overall Height (in)":
  overallHeightRange.min !== null &&
  overallHeightRange.max !== null &&
  overallHeightRange.max > overallHeightRange.min
    ? `${fmtNum(overallHeightRange.min)} to ${fmtNum(overallHeightRange.max)}`
    : fmtNum(overallHeightRange.min),

"Overall Depth (in)":
  overallDepthDec !== null ? fmtNum(overallDepthDec) : "N/A",
