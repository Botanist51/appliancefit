...
function pickCutoutFromSpecSection($) {
  const out = {
    width: null,
    height: null,
    depth: null
  ...
  // Find the "Cut Out Dimensions" section header
  const header = $("tr, div")
    .filter((_, el) =>
  ...
  // Read the next few rows only
  for (let i = 0; i < 6 && row.length; i++) {
    const label = row.find("td, th").first().text().toLowerCase().trim();
    const value = row.find("td, th").last().text().trim();
...
// Second fallback: Cut Out Dimensions section
if (!cutoutW || !cutoutH || !cutoutD) {
  const cutoutSection = pickCutoutFromSpecSection($);
  if (!cutoutW) cutoutW = cutoutSection.width;
  if (!cutoutH) cutoutH = cutoutSection.height;
  if (!cutoutD) cutoutD = cutoutSection.depth;
}
