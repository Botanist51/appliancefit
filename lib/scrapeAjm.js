...
function extractDimsFromQuickSpecs($) {
  ...
  root.find("span.bold").each((_, el) => {
    ...
    const isWidth = labelKey === "width" || labelKey === "overall width" || labelKey === "product width";
    const isHeight = labelKey === "height" || labelKey === "overall height" || labelKey === "product height";
    const isDepth = labelKey === "depth" || labelKey === "overall depth" || labelKey === "product depth";
...
export async function scrapeAjmModel(rawModel) {
  ...
  if (!overallW || !overallH || !overallD) {
    ...
    const mainLabelMatchers = {
      width: ["width", "overall width", "product width", "unit width"],
      height: ["height", "heigh", "overall height", "product height", "unit height"],
      depth: ["depth", "overall depth", "product depth", "unit depth"]
    };
  ...
  // FINAL fallback: Quick Specs (dishwashers, compact units)
if (!overallW || !overallH || !overallD) {
