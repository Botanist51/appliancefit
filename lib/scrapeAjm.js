...
function extractDimsFromMainPage($, sectionTitles, labelMatchers) {
  ...
  let header = null;
  root
    .find("h1,h2,h3,h4,div.bold,div.font-bold,strong,div.font-weight-bold,span.bold")
    .each((_, el) => {
      const t = normKey($(el).text());
      if (sectionTitleKeys.includes(t)) {
        header = $(el);
  ...
  let startNode = header ? header.next() : root.find("div#specs-summary, div.product-specs, div.sidebar-specs").first();  // Fallback to common sidebar classes if no header
  if (!startNode || !startNode.length) startNode = root;  // Ultimate fallback to whole root
  let node = startNode;
  while (node.length) {
    const isNewHeader =
      node.is("h1,h2,h3,h4") ||
...
export async function scrapeAjmModel(rawModel) {
  ...
  // First, try Specifications tab
  let specsDims = extractOverallDimensions($);
  overallW = specsDims.width || overallW;
  overallH = specsDims.height || overallH;
  overallD = specsDims.depth || overallD;

  // If any is missing from specs, fill from main page
  ...
  if (!overallW) overallW = mainDims.width;
  if (!overallH) overallH = mainDims.height;
  if (!overallD) overallD = mainDims.depth;
