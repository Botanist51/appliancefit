...
function extractOverallDimensionsFromSpecsTabClone($) {
  ...
  // If AJM used HxWxD instead of Width/Height/Depth rows
  if (!out.width || !out.height || !out.depth) {
    const root = $('div.tab-pane[id$="-specs-tab-pane"]').length
      ? $('div.tab-pane[id$="-specs-tab-pane"]')
      : $("body");

    root.find("div.spec-bar").each((_, el) => {
      if (out.width && out.height && out.depth) return;

      const label = normKey($(el).find("div.col").first().text());
      const value = $(el).find("div.col").last().text();

      if (label.includes("hxwxd") || label.includes("dimensions")) {
        const split = splitHxWxD(value);
        if (split) {
          out.height = split.height;
          out.width = split.width;
          out.depth = split.depth;
        }
      }
    });
  }
