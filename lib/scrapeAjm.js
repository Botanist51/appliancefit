...
function extractManualsAndGuides($) {
  ...
  const container = $('h2')
    ...
    .first()
    .parent(); // ← THIS is the correct anchor

  if (!container.length) return null;

  container.children(".block").each((_, block) => {
    const link = $(block).find("a[href$='.pdf']").first();
    if (!link.length) return;

    const title = normText(link.text());
    ...
    // 🚨 FIRST VALID DOCUMENT WINS — GUARANTEED ORDER
    if (!primary) {
      primary = entry;
    }
  });
