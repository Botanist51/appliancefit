...
function extractDimsFromMainPage($, sectionTitles, labelMatchers) {
  ...
  // Expanded header find to catch more AJM variations (e.g., gray headers)
  let header = null;
  root
    .find("h1,h2,h3,h4,div.bold,div.font-bold,strong,div.font-weight-bold,span.bold,div.bg-gray,div.spec-title,div[class*='header'],div[class*='title']")
    .each((_, el) => {
      const t = normKey($(el).text());
      if (sectionTitleKeys.includes(t)) {
        header = $(el);
  ...
  // If no header found, fallback to common sidebar containers or full root scan
  let node;
  if (header && header.length) {
    node = header.next();
  } else {
    // Fallback to likely sidebar divs
    const sidebar = root.find("div#product-specs, div.specs-sidebar, div.quick-specs, div.product-info, div.sidebar").first();
    node = sidebar.length ? sidebar.children().first() : root.children().first();
  }
  while (node.length) {
    const isNewHeader = node.is("h1,h2,h3,h4") ||
      node.is("div.bold") ||
      node.hasClass("bold") ||
    ...
    if (labelKey && value) {
      // NEW: Add skip here to prevent cutout/exterior in col/li/tr paths
      const isExterior = labelKey.includes("exterior") || labelKey.includes("nominal");
      const isCutout = labelKey.includes("cutout") || labelKey.includes("cut out") || labelKey.includes("cavity") || labelKey.includes("niche") || labelKey.includes("opening");
...
export async function scrapeAjmModel(rawModel) {
  ...
  if (!overallW || !overallH || !overallD) {
    ...
    if (!overallH && mainDims.height) overallH = overall.height;
...
"Overall Height (in)":
  overallHeightRange.min !== null && overallHeightRange.max !== null && overallHeightRange.max > overallWidthRange.min
