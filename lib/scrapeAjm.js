...
function pickCutoutFromSpecBlocks($) {
  const out = { width: null, height: null, depth: null };

  // Find the text node that says "Cut Out Dimensions"
  const header = $("*")
    .filter((_, el) =>
      $(el).children().length === 0 &&
      $(el).text().trim().toLowerCase() === "cut out dimensions"
    )
    .first();

  if (!header.length) return out;

  // Walk forward through sibling elements
  let el = header.next();

  while (el.length) {
    const text = el.text().toLowerCase();

    if (text.includes("dimensions") && !text.includes("cut out")) break;

    const label = el.find("*").first().text().trim().toLowerCase();
    const value = el.find("*").last().text().trim();
...
let cutoutD = pick(pageText, "Cutout Depth");
  ...
  const cutoutBlock = pickCutoutFromSpecBlocks($);
  if (!cutoutW) cutoutW = cutoutBlock.width;
  if (!cutoutH) cutoutH = cutoutBlock.height;
  if (!cutoutD) cutoutD = cutoutBlock.depth;
}
