...
function extractDimsFromMainPage($, sectionTitles, labelMatchers) {
  ...
  // We intentionally exclude the specs tab pane
  ...
  const matches = (labelKey, matcher) => {
    if (Array.isArray(matcher)) return matcher.every(t => labelKey.includes(normKey(t)));
    return labelKey.includes(normKey(matcher));
  };

  // ✅ IMPORTANT: header match is "contains", not exact equals
  let header = null;
  root.find("h1,h2,h3,h4,div.bold,div.font-bold,strong").each((_, el) => {
    const t = normKey($(el).text());
    if (sectionTitleKeys.some(k => t.includes(k))) {
      header = $(el);
      return false;
    }
  });
  ...
  // ✅ Instead of walking siblings, search inside a reasonable container
  let container =
    header.closest("section,article,div").first();

  if (!container.length) container = root;

  // Collect candidate “rows” from common AJM layouts
  const rows = [];

  // spec-bar rows
  container.find("div.spec-bar").each((_, el) => rows.push($(el)));
  ...
  container.find("dl").each((_, dl) => {
    ...
    dts.each((i, dt) => {
      const dd = $(dt).next("dd");
      if (dd.length) rows.push($(dt));
    });
  });

  // li "Label: value" rows
  container.find("li").each((_, el) => rows.push($(el)));

  // table rows
  container.find("tr").each((_, el) => rows.push($(el)));

  for (const node of rows) {
    let labelRaw = "";
    let valueRaw = "";

    if (node.hasClass("spec-bar") || node.children("div.col").length >= 2) {
      const cols = node.children("div.col");
      labelRaw = cols.first().text();
      valueRaw = cols.last().text();
    } else if (node.is("dt")) {
      labelRaw = node.text();
      valueRaw = node.next("dd").text();
    } else if (node.is("li")) {
      const txt = node.text();
