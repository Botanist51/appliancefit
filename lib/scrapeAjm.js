...
// Update extractDimsFromSpecsTab to add skips for overall context (add this inside the function, but only when called for overall; for simplicity, since cutout has its own specific matchers, we can apply conditionally or always—here always as cutout matchers require "cutout" anyway)
function extractDimsFromSpecsTab($, sectionTitles, labelMatchers) {
  ...
  while (node.length) {
    ...
    if (node.hasClass("spec-bar")) {
      ...
      // NEW: Skip unwanted labels for overall/product dims (universal guard against exterior/nominal and cutout bleed)
      ...
      // (Existing HxWxD split handling remains)
...
function extractDimsFromMainPage($, sectionTitles, labelMatchers) {
  ...
  // Expanded header find to catch more variations, including plain text-like headers
  let header = null;
  root
    .find("h1,h2,h3,h4,div.bold,div.font-bold,div.font-weight-bold,strong,span.bold,div.spec-heading,p,strong")
    .each((_, el) => {
      const t = normKey($(el).text());
      if (sectionTitleKeys.includes(t)) {
        header = $(el);
  ...
  let node = header ? header.next() : root;  // If no header, scan full root as fallback
  while (node.length) {
    const isNewHeader =
      node.is("h1,h2,h3,h4") ||
      node.is("div.bold") ||
      ...
      node.hasClass("font-weight-bold") ||
      normKey(node.text()).includes('capacity') || normKey(node.text()).includes('appearance');  // Stop at next sections
    if (isNewHeader && header) break;  // Only break on headers if we started from one
    let labelRaw = "";
    let valueRaw = "";
    ...
    } else if (node.is("li") || node.is("p") || node.is("div") || node.type === 'text') {  // NEW: Handle plain text nodes/lines
...
export async function scrapeAjmModel(rawModel) {
  ...
  // NEW: Fallback to main page if still missing any overall dims
  if (!overallW || !overallH || !overallD) {
    const sectionTitles = [ // Reuse similar titles for main page
      "Dimensions",
      "Product Dimensions",
      "Overall Dimensions",
      "Exterior Dimensions",
    ...
    const labelMatchers = { // Reuse same as specs (exteriors already removed)
      width: ["width", "overall width", "product width", "unit width"],
      height: ["height", "overall height", "product height", "unit height"],
      depth: ["depth", "overall depth", "product depth", "unit depth"]
    };
    const mainDims = extractDimsFromMainPage($, sectionTitles, labelMatchers);
  ...
  // Removed unreliable last-resort picks for "Product Width" / "Width" etc.
