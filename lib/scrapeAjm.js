      ...
      if (!(result.width && result.height && result.depth)) {
        if (labelKey.includes("dimensions") && labelKey.includes("h") && labelKey.includes("w") && labelKey.includes("d")) {
          ...
          if (split) {
            ...
            // After main extraction loop, if width is still null or looks like cutout-ish, fallback to exterior
if (!result.width || result.width.includes('cutout') || parseInches(result.width) > 30) { // rough guard
  // search again with exterior matchers
}
            if (labelKey.includes('cutout') || labelKey.includes('opening') || labelKey.includes('cavity') || labelKey.includes('rough')) {
  // skip assignment for overall extraction
  continue;
}  
          }
        }
...
function extractOverallDimensions($) {
  ...
  const labelMatchers = {
    ...
    height: [
      "height",
      "heigh",  // NEW: Handles observed typo on GE pages like PTS7000SNSS
      "overall height",
      "product height",
      "unit height"
    ],
