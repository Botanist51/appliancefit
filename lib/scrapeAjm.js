...
function extractDimsFromSpecsTab($, sectionTitles, labelMatchers) {
  ...
  while (node.length) {
    ...
    if (node.hasClass("spec-bar")) {
      ...
      if (!result.width && wMatchers.some(m => labelKey.includes(normKey(m)))) result.width = value;
      if (!result.height && hMatchers.some(m => labelKey.includes(normKey(m)))) result.height = value;
      if (!result.depth && dMatchers.some(m => labelKey.includes(normKey(m)))) result.depth = value;
    }
...
function extractCutoutDimensions($) {
  ...
  const labelMatchers = {
    width: [
      ...
      "cut out",
      "cavity width",
      "minimum inside width of cabinetry frame",
      "minimum internal width of cabinetry",
    ...
    height: [
      ...
      "cutout height",
      "cavity height",
      "minimum internal height of cabinetry",
    ...
    depth: [
      ...
      "depth minimum",
      "minimum internal depth of cabinetry",
