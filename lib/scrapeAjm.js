...
function extractDimsFromSpecsTab($, sectionTitles, labelMatchers) {
  ...
  const empty = { width: null, height: null, depth: null };
  if (!sectionTitleKeys.length) return empty;

  // AJM section headers are div.bold blocks (often with bg-gray-2)
  const headers = root.find("div.bold");

  // helper: match label against matcher(s)
  ...
  // Collect ALL headers that match any of our section titles (in DOM order)
  const matchingHeaders = [];
  headers.each((_, el) => {
    const t = normKey($(el).text());
    if (sectionTitleKeys.includes(t)) matchingHeaders.push($(el));
  });

  if (!matchingHeaders.length) return empty;

  // Try each matching section; return the first one that yields all 3 dims.
  // Otherwise, keep the "best" partial result.
  let best = { width: null, height: null, depth: null };
  ...
  for (const header of matchingHeaders) {
    ...
    // If this section gave us more info than best, keep it
    const score = (o) => (o.width ? 1 : 0) + (o.height ? 1 : 0) + (o.depth ? 1 : 0);
    if (score(result) > score(best)) best = result;

    // Perfect hit: stop immediately
    if (best.width && best.height && best.depth) return best;
  }
...
function extractDimsFromMainPage($, sectionTitles, labelMatchers) {
  ...
  while (node.length) {
    ...
    if (labelKey && value) {
      ...
      // We store RAW value (not normText) so ranges like `33 15/16" - 34 15/16"` stay intact
      if (!result.width && wMatchers.some(m => matches(labelKey, m))) result.width = value;
      if (!result.height && hMatchers.some(m => matches(labelKey, m))) result.height = value;
      if (!result.depth && dMatchers.some(m => matches(labelKey, m))) result.depth = value;
...
function extractOverallDimensionsFromSpecsTabClone($) {
  const sectionTitles = [
    "Dimensions",
    "Product Dimensions",
    "Overall Dimensions",
  ...
  const labelMatchers = {
    width: ["width", "overall width", "product width", "exterior width", "unit width"],
    height: ["height", "overall height", "product height", "exterior height", "unit height"],
    depth: ["depth", "overall depth", "product depth", "exterior depth", "unit depth"]
  };

  const out = extractDimsFromSpecsTab($, sectionTitles, labelMatchers);

    // If AJM used HxWxD instead of Width/Height/Depth rows (ONLY within matching sections)
  if (!out.width || !out.height || !out.depth) {
    const root = $('div.tab-pane[id$="-specs-tab-pane"]').length
      ? $('div.tab-pane[id$="-specs-tab-pane"]')
      : $("body");

    const sectionTitleKeys = sectionTitles.map(t => normKey(t));
    const headers = root.find("div.bold");
    ...
    for (const header of matchingHeaders) {
      ...
      while (node.length) {
        ...
        if (node.hasClass("spec-bar")) {
          const label = normKey(node.find("div.col").first().text());
          const value = node.find("div.col").last().text();

          if (label.includes("hxwxd") || label === "dimensions") {
            const split = splitHxWxD(value);
            if (split) {
              if (!out.height) out.height = split.height;
              if (!out.width) out.width = split.width;
              if (!out.depth) out.depth = split.depth;
            }
          }
        }
...
// Same pipeline as Cutout, but using the cloned Overall engine
const overall = extractOverallDimensionsFromSpecsTabClone($);
overallW = overall.width || null;
overallH = overall.height || null;
overallD = overall.depth || null;
