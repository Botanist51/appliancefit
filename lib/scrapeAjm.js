...
function extractDimsFromMainPage($, sectionTitles, labelMatchers) {
  ...
  while (node.length) {
    ...
    if (node.children("div.col").length >= 2) {
      ...
      // NEW: Catch plain text nodes or div/p with "Label: Value" format (common in AJM sidebars for dishwashers)
...
export async function scrapeAjmModel(rawModel) {
  ...
  // NEW: Fallback to structured extraction from main page sidebar if specs tab didn't yield all dims
  if (!overallW || !overallH || !overallD) {
  ...
  // Last-resort pageText aliases (covers cases where structured extraction misses)
